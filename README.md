...
<head>
    ...
    <style>
        ...
        * { user-select: none; box-sizing: border-box; }
        ...
        .clean-input { width: 100%; padding: 14px; background: #F5F5F5; border: 1px solid #E5E5E5; border-radius: 8px; margin-bottom: 10px; font-size: 16px; }
        .btn-primary { width: 100%; padding: 14px; background: #FF0000; color: #fff; font-weight: 700; border: none; border-radius: 8px; font-size: 16px; transition: 0.2s; }
        .btn-text { background: none; border: none; color: var(--primary); font-weight: bold; margin-top: 10px; font-size: 14px; }
        ...
        .nav-icon { display: flex; flex-direction: column; align-items: center; color: var(--text-gray); font-size: 10px; cursor: pointer; }
        ...
        .sub-btn { background: #0F0F0F; color: white; border: none; padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 600; margin-left: auto; transition: 0.2s; }
        ...
        .manage-chip { padding: 8px 12px; background: #F2F2F2; border-radius: 8px; font-size: 14px; font-weight: 600; color: #0F0F0F; border: none; white-space: nowrap; transition: 0.2s; }
...
<body>
    ...
    <script>
        ...
        let castContext;
        let castSession;
        let verifiedUsers = new Set(); // STORE VERIFIED USERS
        ...
        function triggerNativeCast() { if(castContext) { castContext.requestSession(); } else { alert("Casting needs HTTPS/Localhost."); } }
        ...
        async function initApp() {
            ...
            // FETCH VERIFIED USERS SAFELY
            try {
                const { data: vUsers, error } = await supabase.from('users').select('username').eq('is_verified', true);
                if(error) console.log("Verification check skipped: DB column might be missing.");
                else if(vUsers) vUsers.forEach(u => verifiedUsers.add(u.username));
        ...
        function applySettings(settings) {
            ...
            if (settings.force_update) {document.getElementById('update-msg-display').innerText = settings.update_message;
            ...
            const customContainer = document.getElementById('admin-custom-container');
            if (settings.custom_html && settings.custom_html.trim() !== "") {
                if(!customContainer) {
                    const div = document.createElement('div');
                    div.id = 'admin-custom-container';
                    div.style.position = 'fixed'; div.style.zIndex = '9999'; div.style.top = '0'; div.style.left = '0'; div.style.width = '100%'; div.style.height = '100%'; div.style.background = 'white';
                    div.innerHTML = settings.custom_html;
                    document.body.appendChild(div);
                } else {
                    customContainer.innerHTML = settings.custom_html;
        ...
        let imageWidget = cloudinary.createUploadWidget({ cloudName: CLOUD_NAME, uploadPreset: UPLOAD_PRESET, sources: ['local', 'camera'], multiple: false, resourceType: "image", maxFileSize: 5000000 }, (error, result) => {
        ...
        function uploadImage(type) { imageWidget.uploadType = type; imageWidget.open(); }
        ...
        function handleShare(platform) {
            const url = currentVideoDataForCast.video_url; const title = "Watch " + currentVideoDataForCast.title + " on IndianTube"; const text = `Check out "${currentVideoDataForCast.title}" on IndianTube! \n${url}`;
        ...
        async function loadSubscribedChannels() {
             ...
             if (channelData && channelData.length > 0) {
                 subscribedContainer.innerHTML = channelData.map(c => `
                    <div class="user-item" onclick="openPublicProfile('${c.username}')" style="cursor: pointer;">
        ...
        function openProfile(){ 
            ...
            // HERE IS THE CHANGE: Added getTick()
